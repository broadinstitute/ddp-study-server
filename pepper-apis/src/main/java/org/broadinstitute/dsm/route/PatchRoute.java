                        ...
                        for (NameValue nameValue : patch.getNameValues()) {
                            ...
                            if (dbElement != null) {
                                ...
                                if (nameValue.getName().indexOf("question") > -1) {
                                    ...
                                    JsonObject jsonObject = new JsonParser().parse(nameValue.getValue().toString()).getAsJsonObject();
                                    JsonArray questionArray = jsonObject.get("questions").getAsJsonArray();
                                    boolean writeBack = false;
                                    for (int i = 0; i < questionArray.size(); i++) {
                                        JsonObject question = questionArray.get(i).getAsJsonObject();
                                        if (question.get("status") != null && question.get("status").getAsString().equals("sent")) {
                                            if (question.get("email") != null && question.get("question") != null) {
                                                notificationUtil.sentAbstractionExpertQuestion(user.getEmail(), user.getName(), question.get("email").getAsString(),
