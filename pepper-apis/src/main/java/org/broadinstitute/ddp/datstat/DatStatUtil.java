...
import org.broadinstitute.ddp.PortalServer;
...
public class DatStatUtil implements EDCClient
{
    ...
    //monitoring
    private static final AtomicInteger dupSurveySessionErrors = new AtomicInteger(0);
    private static final NumberGauge dupSurveySessionErrorGauge = new NumberGauge(MonitorConfig.builder("dup_surveysesssion_gauge").build(), dupSurveySessionErrors);
    private static final AtomicInteger responseTime = new AtomicInteger(0);
    private static final NumberGauge datStatResponseGauge = new NumberGauge(MonitorConfig.builder("datstat_response_gauge").build(), responseTime);
    private static final AtomicInteger datStatSyncHealthy = new AtomicInteger(0);
    private static final NumberGauge datStatSyncHealthyGauge = new NumberGauge(MonitorConfig.builder("ds_sync_ok_gauge").build(), datStatSyncHealthy);

    //explicitly wire up the metrics using a static initializer
    static {
        DefaultMonitorRegistry.getInstance().register(dupSurveySessionErrorGauge);
        DefaultMonitorRegistry.getInstance().register(datStatResponseGauge);
        DefaultMonitorRegistry.getInstance().register(datStatSyncHealthyGauge);
    }
    ...
    public synchronized void startup(@NonNull Config config, Scheduler scheduler)
    {
        ...
        configureInstitutionSurveyId(config);
    ...
    public HttpResponse sendRequest(@NonNull MethodType method, @NonNull String additionalPath, Object postData)
    {
        ...
        if (postData != null) content = new JsonHttpContent(new GsonFactory(), postData);
    ...
    private HttpResponse sendSingleRequest(@NonNull Boolean resetValues, @NonNull MethodType method, @NonNull String additionalPath, HttpContent content) throws Exception
    {
        ...
        responseTime.set((int)(System.currentTimeMillis()-startTime));
    ...
    private void configureInstitutionSurveyId(Config config) {
        if (config.getStringList("portal.ddpRouteTypes").stream().anyMatch(str -> str.equals(PortalServer.DDPRouteTypes.MR.toString()))) {
            institutionSurveyId = getLastIdFromUri(getSurveyConfigMap().get(config.getString("datStat.institutionSurveyPath")).getSurveyDefinition().getUri());
        }
    }
    ...
    public ArrayList<String> getSurveySessionsViaUri(String surveyURI, String participantAltPid, boolean allowMultipleSessions,
        ...
        try {
            ...
            if ((!allowMultipleSessions)&&(resultList.size() > 1)) {
                dupSurveySessionErrors.incrementAndGet();
