...
public class JavaProcessSpawner {
     ...
     * @throws IOException
     */
    public static Future<ProcessResult> spawnMainInSeparateProcess(Class classWithMainMethod, Class sendLogsTo, int bootTimeInSeconds) throws IOException {
        ...
        if (bootTimeInSeconds < 1) {
            logger.warn("With a boot wait time of " + bootTimeInSeconds + ", clients of " + classWithMainMethod + " may discover that the process the need has not been started yet.");
        }
