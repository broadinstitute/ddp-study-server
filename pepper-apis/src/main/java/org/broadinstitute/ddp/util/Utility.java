...
public class Utility
{
    ...
    public static Config getApplicationConfig(@NonNull String[] args)
    {
        ...
        //configure DB ssl now if we need to before doing anything else
        if (!(mergedConfig.hasPath("portal.dbSkipSsl"))||(!mergedConfig.getBoolean("portal.dbSkipSsl"))) {
            TransactionWrapper.configureSslProperties(mergedConfig.getString("portal.dbSslKeyStore"),
                    mergedConfig.getString("portal.dbSslKeyStorePwd"), mergedConfig.getString("portal.dbSslTrustStore"),
                    mergedConfig.getString("portal.dbSslTrustStorePwd"));
        }
        else {
            logger.warn("Skipping DB SSL system settings.");
        }
