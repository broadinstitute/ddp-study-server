...
public class TemplateRenderUtil {
    ...
    public static String applyRenderedTemplate(Long templateId, String templateText,
                                               Renderable.Provider<String> rendered, ContentStyle style) {
        if (templateId != null) {
            templateText = rendered.get(templateId);
            if (templateText == null) {
                throw new NoSuchElementException("No rendered template found for placeholder with id " + templateId);
            }
            if (style == ContentStyle.BASIC) {
