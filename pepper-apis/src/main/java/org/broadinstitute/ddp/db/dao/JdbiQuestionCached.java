...
public class JdbiQuestionCached extends SQLObjectWrapper<JdbiQuestion> implements JdbiQuestion {
    ...
    private static Cache<Long, List<Long>> compositeParentIdToChildIdsCache;
    private static Cache<Long, Long> compositeChildIdToParentIdCache;
        ...
        synchronized (getClass()) {
            ...
            if (compositeParentIdToChildIdsCache == null) {
                compositeParentIdToChildIdsCache = cacheService
                        .getOrCreateCache("ompositeParentIdToChildIdsCache", new Duration(MINUTES, 15), ModelChangeType.STUDY);
            }
            if (compositeChildIdToParentIdCache == null) {
                compositeChildIdToParentIdCache = cacheService
                        .getOrCreateCache("compositeChildIdToParentIdCache", new Duration(MINUTES, 15), ModelChangeType.STUDY);
            }
        }
    }
    ...
    @Override
    public Map<Long, List<Long>> collectOrderedCompositeChildIdsByParentIds(Set<Long> parentQuestionIds) {
    ...
    @Override
    public Stream<CompositeIdPair> findOrderedCompositeChildIdsByParentIds(Set<Long> parentQuestionIds) {
    ...
    public Stream<QuestionDto> findQuestionDtosByIds(Set<Long> questionIds) {
        if (isNullCache(questionIdToDtoCache)) {
            ...
            if (!missingQuestionIds.isEmpty()) {
                var moreQuestionDtos = delegate.findQuestionDtosByIds(missingQuestionIds)
