...
public interface JdbiAuth0LogEvent extends SqlObject {
    ...
    @SqlUpdate("insert into auth0_log_event "
            + "(tenant,log_id,date,type,client_id,connection_id,user_id,user_agent,ip,email,data) "
            + "values(:tenant,:logId,:date,:type,:clientId,:connectionId,:userId,:userAgent,:ip,:email,:data)"
    )
    @GetGeneratedKeys
