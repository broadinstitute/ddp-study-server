...
public class FormActivityServiceTest extends TxnAwareBaseTest {
    ...
    public void testGetBlockVisibilities_withoutExpr_excluded() {
        TransactionWrapper.useTxn(handle -> {
            ...
            List<BlockVisibility> visibilities = service.getBlockVisibilities(handle, def, testData.getUserGuid(), instanceGuid);
    ...
    public void testGetBlockVisibilities_withExpr_included() {
        ...
        TransactionWrapper.useTxn(handle -> {
            setupActivityAndInstance(handle);
            List<BlockVisibility> visibilities = formService.getBlockVisibilities(handle, def, testData.getUserGuid(), instanceGuid);
            assertNotNull(visibilities);
            assertEquals(2, visibilities.size());
    ...
    public void testGetBlockVisibilities_withExpr_evalError() {
        ...
        TransactionWrapper.useTxn(handle -> {
            setupActivityAndInstance(handle);
            formService.getBlockVisibilities(handle, def, testData.getUserGuid(), instanceGuid);
    ...
    private void setupActivityAndInstance(Handle handle) {
        ...
        def = FormActivityDef.generalFormBuilder(actCode, "v1", testData.getStudyGuid())
        ...
        handle.attach(ActivityDao.class).insertActivity(def, RevisionMetadata.now(testData.getUserId(), "test"));
        assertNotNull(def.getActivityId());
