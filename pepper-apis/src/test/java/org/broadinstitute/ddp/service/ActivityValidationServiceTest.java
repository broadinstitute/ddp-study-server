...
public class ActivityValidationServiceTest extends TxnAwareBaseTest {
    ...
    private static void setupActivityAndInstance(Handle handle) throws Exception {
        ...
        // Inserting answers
        long answerId1 = handle.attach(JdbiAnswer.class).insertBaseAnswer(txt1.getQuestionId(), userGuid, instanceDto.getId());
        long answerId2 = handle.attach(JdbiAnswer.class).insertBaseAnswer(txt2.getQuestionId(), userGuid, instanceDto.getId());
        handle.attach(JdbiTextAnswer.class).insert(answerId1, "answer to question 1");
        handle.attach(JdbiTextAnswer.class).insert(answerId2, "answer to question 2");
