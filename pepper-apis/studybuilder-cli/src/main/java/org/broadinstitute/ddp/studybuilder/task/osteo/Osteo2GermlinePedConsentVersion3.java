...
@Slf4j
public class Osteo2GermlinePedConsentVersion3 implements CustomTask {

    private static final String DATA_FILE = "patches/germline-consent-addendum-ped-v3.conf";
    ...
    private static final String BLOCK_ADD_PAR = "block-add-parent";
    private static final String BLOCK_ADD_CHD = "block-add-child";
    private static final String OLD_TEMPLATE_KEY = "old_template_search_text";
    private static final String DATE_QUESTION_STABLEID_PAR = "ADDENDUM_CONSENT_DOB_PEDIATRIC";
    private static final String DATE_QUESTION_STABLEID_CHD = "ADDENDUM_CONSENT_PATIENT_DOB_PEDIATRIC";
    ...
    public void run(Handle handle) {
        log.info("Executing task: Osteo2GermlinePedConsentVersion3");
        ...
        runGermlinePedConsentUpdate(handle, metaConsent, version3ForConsent, dataCfg);

        //TODO
        //disable parent/guardian Date Question & add new Content block with sys date
        disableQuestionAndAddContent(handle, studyDto.getId(), metaConsent, version3ForConsent, DATE_QUESTION_STABLEID_PAR, BLOCK_ADD_PAR);

        //disable Child Date Question & add new Content block with sys date
        disableQuestionAndAddContent(handle, studyDto.getId(), metaConsent, version3ForConsent, DATE_QUESTION_STABLEID_CHD, BLOCK_ADD_CHD);
    }
    ...
    private void runGermlinePedConsentUpdate(Handle handle, RevisionMetadata meta, ActivityVersionDto version, Config dataCfg) {
        updateTemplates(handle, meta, version, dataCfg); //revision full Template
    ...
    private void disableQuestionAndAddContent(Handle handle, Long studyId, RevisionMetadata meta, ActivityVersionDto versionDto,
                                 String questionStableId, String blockToAdd) {
        ...
        QuestionDto dateDto = jdbiQuestion.findLatestDtoByStudyIdAndQuestionStableId(studyId, questionStableId).get();
        long dateBlockId = this.sqlHelper.findQuestionBlockId(dateDto.getId());
        SectionBlockMembershipDto dateSectionDto = jdbiFormSectionBlock.getActiveMembershipByBlockId(dateBlockId).get();
        SectionBlockDao sectionBlockDao = handle.attach(SectionBlockDao.class);
        sectionBlockDao.disableBlock(dateBlockId, meta);
