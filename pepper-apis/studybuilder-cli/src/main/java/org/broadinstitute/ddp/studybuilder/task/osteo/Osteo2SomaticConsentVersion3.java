...
@Slf4j
public class Osteo2SomaticConsentVersion3 implements CustomTask {

    private static final String DATA_FILE = "patches/somatic-consent-addendum-v3.conf";
    ...
    public void run(Handle handle) {
        ...
        ActivityVersionDto activityVer = getversion3(handle, studyDto, metaConsent, activityCode, versionTag);
    ...
    private void activityUpdate(Handle handle, RevisionMetadata meta, ActivityVersionDto version3) {
        updateTemplates(handle, meta, version3); //revision full Template
    }

    private void updateTemplates(Handle handle, RevisionMetadata meta, ActivityVersionDto version3) {
        log.info("UPDATE Templates/Content Blocks");
        List<? extends Config> configList = dataCfg.getConfigList(BLOCK_UPDATES);
        for (Config config : configList) {
            revisionContentBlockTemplate(handle, meta, version3, config);
        }
    }
    ...
    private void revisionContentBlockTemplate(Handle handle, RevisionMetadata meta, ActivityVersionDto versionDto, Config conf) {
        ...
        BlockContentDto contentBlock = handle.attach(Osteo2SomaticConsentVersion3.SqlHelper.class)
