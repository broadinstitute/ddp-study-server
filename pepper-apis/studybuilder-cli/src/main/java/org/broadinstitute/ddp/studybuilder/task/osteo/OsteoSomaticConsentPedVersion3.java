...
@Slf4j
public class OsteoSomaticConsentPedVersion3 implements CustomTask {

    private static final String DATA_FILE = "patches/somatic-consent-addendum-ped-v3.conf";
    ...
    public void run(Handle handle) {
        ...
        ActivityVersionDto activityVer = getVersion3(handle, studyDto, activityCode, versionTag);
        RevisionMetadata metaConsent = new RevisionMetadata(activityVer.getRevStart(), adminUser.getId(), reason);
        activityUpdate(handle, metaConsent, activityVer);
    }

    private ActivityVersionDto getVersion3(Handle handle, StudyDto studyDto, String activityCode,
                                           String versionTag) {
        return handle.attach(JdbiActivityVersion.class)
                .findByActivityCodeAndVersionTag(studyDto.getId(),
                activityCode, versionTag).orElseThrow(DDPException::new);
    }

    private void activityUpdate(Handle handle, RevisionMetadata meta, ActivityVersionDto version) {
    ...
    private void revisionContentBlockTemplate(Handle handle, RevisionMetadata meta, ActivityVersionDto versionDto, Config conf) {
        ...
        BlockContentDto contentBlock = handle.attach(OsteoSomaticConsentPedVersion3.SqlHelper.class)
