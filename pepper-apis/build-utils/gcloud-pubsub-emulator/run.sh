...
# wait-for-it scripts.
(/usr/bin/wait-for "localhost:$1" -- env PUBSUB_EMULATOR_HOST="localhost:$1" /usr/bin/pubsubc -debug; nc -lkp 8682 >/dev/null) &

# Start the PubSub emulator in the foreground.
gcloud beta emulators pubsub start "--host-port=0.0.0.0:$1"
