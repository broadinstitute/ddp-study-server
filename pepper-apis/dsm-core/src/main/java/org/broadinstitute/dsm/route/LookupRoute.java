...
public class LookupRoute extends RequestHandler {
    ...
    private static final String SQL_SELECT_COLLABORATOR_PREFIX =
            "SELECT collaborator_id_prefix FROM ddp_instance WHERE instance_name LIKE ?";
    ...
    private static final String SQL_SELECT_TYPE = "SELECT DISTINCT type_px "
            ...
            + "WHERE oD.type_px LIKE ? AND gr.ddp_group_id = ?";
    ...
    private static final String SQL_SELECT_HISTOLOGY =
            "SELECT DISTINCT histology FROM ddp_onc_history_detail onc, ddp_medical_record rec, ddp_institution inst, ddp_participant part,"
                    + " ddp_instance realm WHERE onc.medical_record_id = rec.medical_record_id "
                    + "AND rec.institution_id = inst.institution_id AND inst.participant_id = part.participant_id "
                    + "AND NOT rec.deleted <=> 1 "
