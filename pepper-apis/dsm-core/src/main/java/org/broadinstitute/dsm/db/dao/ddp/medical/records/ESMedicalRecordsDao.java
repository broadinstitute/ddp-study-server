...
public class ESMedicalRecordsDao implements Dao<ESMedicalRecordsDto> {
    ...
    public static final String SQL_SELECT_ES_MEDICAL_RECORD =
            "SELECT  dp.ddp_participant_id,  mr.medical_record_id,  mr.name,  di.type,  mr.mr_received, "
                    + "mr.fax_sent  FROM  ddp_medical_record mr  LEFT JOIN "
        ...
        SimpleResult results = inTransaction((conn) -> {
            ...
            try (PreparedStatement stmt = conn.prepareStatement(SQL_SELECT_ES_MEDICAL_RECORD + BY_INSTANCE_ID)) {
                ...
                try (ResultSet ESmrRs = stmt.executeQuery()) {
                    while (ESmrRs.next()) {
                        medicalRecordsDtoListES.add(new ESMedicalRecordsDto(ESmrRs.getString(DBConstants.DDP_PARTICIPANT_ID),
                                ESmrRs.getInt(DBConstants.MEDICAL_RECORD_ID), ESmrRs.getString(DBConstants.NAME),
                                ESmrRs.getString(DBConstants.TYPE), ESmrRs.getString(DBConstants.MR_RECEIVED),
