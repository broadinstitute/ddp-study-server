...
public class KitType {
    ...
    private static final String SQL_SELECT_KIT_TYPE =
            "SELECT ks.ddp_instance_id, type.kit_type_name, ks.kit_type_display_name, type.kit_type_id, type.bsp_receptacle_type, "
                    + "type.customs_json, ks.external_shipper "
                    + "FROM ddp_kit_request_settings ks left join kit_type type on (ks.kit_type_id = type.kit_type_id) "
                    + "left join sub_kits_settings subK on (subK.ddp_kit_request_settings_id = ks.ddp_kit_request_settings_id)";
    private static final String SQL_SELECT_CARRIER_SERVICE =
            "SELECT dkc.kit_type_id, kt.kit_type_name, kt.customs_json, ddp_instance_id, cs_to.carrier as carrierTo, "
                    + "cs_to.easypost_carrier_id as carrierToId, "
                    ...
                    + "ret.return_address_zip, ret.return_address_country, ret.return_address_phone, dkc.kit_type_display_name, "
                    + "dkc.external_shipper, dkc.external_name, "
                    ...
                    + "FROM ddp_kit_request_settings dkc LEFT JOIN kit_dimension dim ON (dkc.kit_dimension_id = dim.kit_dimension_id) "
            ...
            try (PreparedStatement stmt = conn.prepareStatement(SQL_SELECT_KIT_TYPE)) {
                try (ResultSet rs = stmt.executeQuery()) {
                    while (rs.next()) {
                        ...
                        kitTypes.put(key, new KitType(rs.getInt(DBConstants.KIT_TYPE_ID), rs.getInt(DBConstants.DDP_INSTANCE_ID),
            ...
            try (PreparedStatement stmt = conn.prepareStatement(SQL_SELECT_CARRIER_SERVICE)) {
                try (ResultSet rs = stmt.executeQuery()) {
                    while (rs.next()) {
                        ...
                        if (kitType != 0) {
                            kitTypes.add(
                                    new KitType(kitType, rs.getInt(DBConstants.DDP_INSTANCE_ID), rs.getString(DBConstants.KIT_TYPE_NAME),
                                            rs.getString(DBConstants.KIT_TYPE_DISPLAY_NAME), rs.getString(DBConstants.EXTERNAL_SHIPPER),
                                            rs.getString(DBConstants.CUSTOMS_JSON), rs.getString("dkc." + DBConstants.EXTERNAL_KIT_NAME)));
                        }
                    }
                }
            } catch (SQLException ex) {
