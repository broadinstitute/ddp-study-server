...
public class GBFRequestUtil implements ExternalShipper {
    ...
    public static final String SQL_SELECT_EXTERNAL_KIT_NOT_DONE = "SELECT * FROM (SELECT kt.kit_type_name, ddp_site.instance_name, "
            + "ddp_site.ddp_instance_id, ddp_site.base_url, ddp_site.auth0_token, ddp_site.migrated_ddp, " +
            "                                    ddp_site.collaborator_id_prefix, req.bsp_collaborator_sample_id, req.ddp_participant_id,"
            + " req.ddp_label, req.dsm_kit_request_id, req.bsp_collaborator_participant_id,  " +
            "                                    req.kit_type_id, req.external_order_status, req.external_order_number, req"
            + ".external_order_date, req.external_response, kt.no_return, req.created_by, kit.kit_label FROM kit_type kt, ddp_kit_request"
            + " req, ddp_kit kit, ddp_instance ddp_site " +
            "                                    WHERE req.ddp_instance_id = ddp_site.ddp_instance_id AND req.kit_type_id = kt"
            + ".kit_type_id AND kit.dsm_kit_request_id = req.dsm_kit_request_id " +
            "                                    and kit.test_result is null and kit.CE_order is null " +
            "                                    and (kit.kit_label is null or kit.tracking_return_id is null or kit.tracking_to_id is "
            + "null)" +
            ") " +
            "                                     AS request" +
            "                                    LEFT JOIN ddp_participant_exit ex ON (ex.ddp_instance_id = request.ddp_instance_id AND "
            + "ex.ddp_participant_id = request.ddp_participant_id)" +
            "                                    LEFT JOIN (SELECT subK.kit_type_id, subK.external_name from ddp_kit_request_settings dkc"
            + " " +
            "                                    LEFT JOIN sub_kits_settings subK ON (subK.ddp_kit_request_settings_id = dkc"
            + ".ddp_kit_request_settings_id)) as subkits ON (subkits.kit_type_id = request.kit_type_id) " +
            "                                WHERE ex.ddp_participant_exit_id is null AND request.ddp_instance_id = ? AND "
            + "(external_order_status not like '%CANCELLED%' or external_order_status is null) AND external_order_number IS NOT NULL " +
    ...
    private static final String SQL_SELECT_KIT_REQUEST_BY_EXTERNAL_ORDER_NUMBER = "SELECT dsm_kit_request_id FROM ddp_kit_request req "
    ...
    private static final String SQL_SELECT_SENT_KIT_FOR_NOTIFICATION_EXTERNAL_SHIPPER = "select  eve.*,   request.ddp_participant_id,   "
            ...
            + "realm.instance_name, realm.base_url, realm.auth0_token, realm.notification_recipients, realm.migrated_ddp, kit"
            + ".receive_date, kit.scan_date" +
            "        from ddp_kit_request request, ddp_kit kit, event_type eve, ddp_instance realm where request.dsm_kit_request_id = kit"
            + ".dsm_kit_request_id and request.ddp_instance_id = realm.ddp_instance_id" +
            "        and (eve.ddp_instance_id = request.ddp_instance_id and eve.kit_type_id = request.kit_type_id) and eve.event_type = "
            + "\"SENT\" and request.external_order_number is not null and request.external_order_number= ?";
    ...
    private final String DISTRIBUTED = "DISTRIBUTION"; //INDICATES: the order is in the final stages of shipping / Can no longer be
    // ‘Cancelled’
    ...
    private String SQL_SELECT_KIT_FOR_NOTIFICATION_EXTERNAL_SHIPPER = "select  eve.*,   request.ddp_participant_id,   request.ddp_label, "
            + "  request.dsm_kit_request_id, request.ddp_kit_request_id, request.upload_reason, " +
            "        realm.ddp_instance_id, realm.instance_name, realm.base_url, realm.auth0_token, realm.notification_recipients, realm"
            + ".migrated_ddp, kit.receive_date, kit.scan_date" +
            "        FROM ddp_kit_request request, ddp_kit kit, event_type eve, ddp_instance realm where request.dsm_kit_request_id = kit"
            + ".dsm_kit_request_id and request.ddp_instance_id = realm.ddp_instance_id" +
            "        and not exists " +
    ...
    private static List<KitRequest> getKitRequest(@NonNull List<KitRequest> kitRequests, @NonNull String externalOrderNumber,
                                                  String subKitName, int tubeNumber) {
        int counter = -1;
    ...
    public static Map getParticipant(DDPInstance ddpInstance, String ddpParticipantId) {
        Map<String, Map<String, Object>> participantsESData = null;
        if (StringUtils.isNotBlank(ddpInstance.getParticipantIndexES())) {
            participantsESData = ElasticSearchUtil.getFilteredDDPParticipantsFromES(ddpInstance,
        ...
        if (kitRequests != null && !kitRequests.isEmpty()) {
            ...
            for (KitRequest kit : kitRequests) {
                ...
                if (!address.isComplete()) {
                    ...
                    Order order = new Order(kit.getExternalOrderNumber(), kitRequestSettings.getExternalClientId(),
                            kit.getParticipantId(), shippingInfo, lineItems);
    ...
    public void orderStatus(Connection conn, KitRequest kit, int instanceId, boolean gbfShippedTriggerDSSDelivered) throws Exception {
        ...
        orderNumbers.addAll(Arrays.asList(kit.getExternalOrderNumber()));
        JsonObject payload = new JsonObject();
                ...
                for (Status status : statuses) {
                    ...
                    KitDDPNotification kitDDPNotification =
                            KitDDPNotification.getKitDDPNotification(SQL_SELECT_SENT_KIT_FOR_NOTIFICATION_EXTERNAL_SHIPPER,
                                    kit.getExternalOrderNumber(), 2);//todo change this to the number of subkits but for now 2 for test
                    // boston works
                    if (status.getOrderStatus().equals(NOT_FOUND) && StringUtils.isNotBlank(kit.getExternalOrderStatus()) && kit.getExternalOrderStatus().equals("NOT FOUND")
                        ...
                        if (dsmKitRequestIds != null && !dsmKitRequestIds.isEmpty()) {
                            for (String dsmKitRequestId : dsmKitRequestIds) {
                                KitRequestExternal.updateKitRequest(conn, status.getOrderStatus(), System.currentTimeMillis(),
                                        dsmKitRequestId);// in order to update time for the  next 24 hour check we need this
                            }
                        }
                        logger.warn("Kit Request with external order number " + kit.getExternalOrderNumber() + "has not been shipped in "
                                + "the last 24 hours! ");//todo pegah uncomment for production
                        ...
                        if (kitDDPNotification != null) {
                            ...
                            if (gbfShippedTriggerDSSDelivered) {
                                KitDDPNotification kitDeliveredNotification = KitDDPNotification.getKitDDPNotification(conn,
                                        ...
                                        new String[] {DELIVERED, String.valueOf(instanceId), kit.getDsmKitRequestId(),
                        ...
                        logger.error("Kit Request with external order number " + kit.getExternalOrderNumber() + "has got cancelled by "
                                + "GBF!");//todo pegah uncomment for production
                    }
    ...
    // Confirmation may include order number, client(participant) ID, outbound tracking number, return tracking number(s), line item(s),
    // kit serial number(s), etc.
