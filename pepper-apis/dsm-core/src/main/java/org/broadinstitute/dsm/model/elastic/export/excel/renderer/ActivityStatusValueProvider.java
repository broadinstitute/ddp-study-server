...
public class ActivityStatusValueProvider implements ValueProvider{
    ...
    public Collection<String> getValue(String esPath, Map<String, Object> esDataAsMap, Alias key, Filter column) {
        List<Map<String, Object>> activities = (List<Map<String, Object>>) esDataAsMap.get("activities");

        return Collections.singletonList(
                activities.stream().map(activity ->
                        String.format("%s : %s", activity.get(ElasticSearchUtil.ACTIVITY_CODE), activity.get(ElasticSearchUtil.STATUS))
