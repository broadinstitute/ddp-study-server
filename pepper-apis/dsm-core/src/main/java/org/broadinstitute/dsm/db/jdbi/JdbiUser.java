...
public interface JdbiUser extends SqlObject {
    ...
    @SqlQuery("SELECT u.guid, u.user_id, concat(p.first_name, \" \", p.last_name) as name, p.email, p.phone as phoneNumber, "
            + "u.auth0_user_id FROM user u left join user_profile p on (u.user_id = p.user_id)  WHERE p.email = :userEmail")
    ...
    @SqlUpdate("INSERT INTO user_profile (user_id, first_name, last_name, phone, email) "
