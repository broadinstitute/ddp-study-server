...
public class GBFRequestUtil implements ExternalShipper {
    ...
    private final String selectKitForNotificationExternalShipperSql = "select  eve.*, request.ddp_participant_id, request.ddp_label, "
    ...
    public static void processingSingleConfirmation(Response gbfResponse, ShippingConfirmation confirmation, int ddpInstanceId)
            throws Exception {
        ...
        if (kitRequests != null && !kitRequests.isEmpty()) {
            ...
            for (KitRequest kit : kitRequests) {
                ...
                if (!address.isComplete()) {
                    ...
                    Order order = new Order(kit.getExternalOrderNumber(), kitRequestSettings.getExternalClientId(), kit.getParticipantId(),
        ...
        if (gbfResponse != null && gbfResponse.isSuccess()) {
            ...
            if (statuses != null && !statuses.isEmpty()) {
                for (Status status : statuses) {
                    ...
                    if (status.getOrderStatus().equals(notFound) && StringUtils.isNotBlank(kit.getExternalOrderStatus())
                        ...
                        if (kitDDPNotification != null) {
                            ...
                            if (gbfShippedTriggerDSSDelivered) {
                                KitDDPNotification kitDeliveredNotification = KitDDPNotification.getKitDDPNotification(conn,
                                        selectKitForNotificationExternalShipperSql + selectByExternalOrderNumber,
