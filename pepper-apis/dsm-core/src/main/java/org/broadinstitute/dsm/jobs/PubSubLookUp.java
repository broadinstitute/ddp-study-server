...
public class PubSubLookUp {
    ...
    public static void writeResultsIntoDB(TestBostonResult testBostonResult, DDPInstance ddpInstance) {
        ...
        DSMTestResult newDsmTestResult =
                new DSMTestResult(testBostonResult.getResult(), testBostonResult.getTimeCompleted(), testBostonResult.isCorrected());
        if (dsmTestResultArray == null) {
            array = new DSMTestResult[] {newDsmTestResult};
        } else {
            array = Arrays.copyOf(dsmTestResultArray,
                    dsmTestResultArray.length + 1); //create new array from old array and allocate one more element
