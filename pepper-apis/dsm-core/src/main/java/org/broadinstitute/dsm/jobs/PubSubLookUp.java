...
public class PubSubLookUp {
    ...
    private static String SELECT_LATEST_RESULT_QUERY = "SELECT kit.test_result FROM ddp_kit_request req LEFT JOIN  ddp_kit kit ON (kit"
            + ".dsm_kit_request_id = req.dsm_kit_request_id) " +
            "WHERE kit.kit_label = ?";
    private static String UPDATE_TEST_RESULT = "UPDATE ddp_kit SET  test_result = ? WHERE dsm_kit_id <> 0 and  dsm_kit_request_id  in "
            + "(select  dsm_kit_request_id from (select * from ddp_kit as something where something.kit_label = ? ) as t  )";
    ...
    private static void notifyPIs(DDPInstance ddpInstance, TestBostonResult testBostonResult, NotificationUtil notificationUtil) {
        ...
        if (testBostonResult.isCorrected()) {
            ...
            message =
                    "A test result has been recorded as corrected for sample Id " + testBostonResult.getSampleId() + " on time " + testBostonResult.getTimeCompleted() + ". Please login to the study manager to review the result.";
        } else if ("POSITIVE".equals(testBostonResult.getResult())) {
            subject = "POSITIVE RESULT: " + testBostonResult.getSampleId();
            message =
                    "A positive viral test has been recorded for sample Id " + testBostonResult.getSampleId() + " on time " + testBostonResult.getTimeCompleted() + ". Please login to the study manager to review the result.";
        }
        if (StringUtils.isNotBlank(subject) && StringUtils.isNotBlank(message)) {
            notificationUtil.sentNotification(ddpInstance.getNotificationRecipient(), message,
                    NotificationUtil.UNIVERSAL_NOTIFICATION_TEMPLATE, subject);
        }
    }
    ...
    private static boolean shouldWriteResultIntoDB(TestBostonResult testBostonResult) {
        ...
        if (!testBostonResult.isCorrected()
            ...
            throw new RuntimeException("A new result for sample id " + testBostonResult.getSampleId() + " that doesn't match the previous"
                    + " one. Date of the new result: " + testBostonResult.getTimeCompleted());
        }
        return true;
    }
    ...
    public static void tellPepperAboutTheNewResults(Connection conn, TestBostonResult testBostonResult) {
        ...
        String query = "select eve.event_name, eve.event_type, request.ddp_participant_id, request.dsm_kit_request_id, request"
                + ".ddp_kit_request_id, request.upload_reason, " +
                ...
                "ddp_kit kit,   event_type eve,   ddp_instance realm   where request.dsm_kit_request_id = kit.dsm_kit_request_id   and "
                + "request.ddp_instance_id = realm.ddp_instance_id   " +
                " and (eve.ddp_instance_id = request.ddp_instance_id   and eve.kit_type_id = request.kit_type_id)   and eve.event_type = "
                + "\"RESULT\" " + // that's the change from the original query
    ...
    public static void writeResultsIntoDB(TestBostonResult testBostonResult) {
        ...
        DSMTestResult newDsmTestResult = new DSMTestResult(testBostonResult.getResult(), testBostonResult.getTimeCompleted(),
                testBostonResult.isCorrected());
        if (dsmTestResultArray == null) {
            array = new DSMTestResult[] {newDsmTestResult};
        } else {
            array = Arrays.copyOf(dsmTestResultArray, dsmTestResultArray.length + 1); //create new array from old array and allocate one
            // more element
        ...
        SimpleResult results = inTransaction((conn) -> {
            ...
            try (PreparedStatement stmt = conn.prepareStatement(UPDATE_TEST_RESULT)) {
                if (StringUtils.isNotBlank(testBostonResult.getResult())
                    ...
                    if (result != 1) {
                        throw new RuntimeException("Error updating test result for kit label" + testBostonResult.getSampleId() + ", "
...
//                        tellPepperAboutTheNewResults(testBostonResult);
        ...
        if (results.resultException != null) {
            throw new RuntimeException("Couldn't update the test results for kit label" + testBostonResult.getSampleId(),
