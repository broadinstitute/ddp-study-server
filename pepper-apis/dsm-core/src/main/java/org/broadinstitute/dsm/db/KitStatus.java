...
public class KitStatus {
    ...
    private static final String SQL_SELECT_SAMPLES_STATUS =
            "SELECT ty.kit_type_name, kit.scan_date, kit.receive_date, kit.tracking_to_id, kit.easypost_shipment_date, "
                    + "cs.carrier as carrierTo FROM ddp_kit_request req LEFT JOIN ddp_kit kit "
                    + "on (kit.dsm_kit_request_id = req.dsm_kit_request_id) "
                    + "LEFT JOIN kit_type ty on (req.kit_type_id = ty.kit_type_id) "
                    + "LEFT JOIN ddp_kit_request_settings ks on (ks.ddp_instance_id = req.ddp_instance_id "
                    + "AND ks.kit_type_id = req.kit_type_id) "
                    + "LEFT JOIN carrier_service cs on (ks.carrier_service_to_id = cs.carrier_service_id) "
                    + "WHERE req.ddp_participant_id = ? AND req.ddp_instance_id = ? AND kit.scan_date IS NOT NULL "
                    + "AND kit.deactivation_reason IS NULL";

    private static final String SQL_SELECT_BATCH_SAMPLES_STATUS =
            "SELECT req.ddp_kit_request_id, req.ddp_participant_id , ty.kit_type_name, kit.scan_date, kit.receive_date, "
                    + "kit.tracking_to_id, kit.easypost_shipment_date, cs.carrier as carrierTo FROM ddp_kit_request req "
                    + "LEFT JOIN ddp_kit kit on (kit.dsm_kit_request_id = req.dsm_kit_request_id) "
                    + "LEFT JOIN kit_type ty on (req.kit_type_id = ty.kit_type_id) "
                    + "LEFT JOIN ddp_kit_request_settings ks on (ks.ddp_instance_id = req.ddp_instance_id "
                    + "AND ks.kit_type_id = req.kit_type_id) "
                    + "LEFT JOIN carrier_service cs on (ks.carrier_service_to_id = cs.carrier_service_id) "
                    + "WHERE req.ddp_instance_id = ? AND kit.scan_date IS NOT NULL AND kit.deactivation_reason IS NULL";
            ...
            try (PreparedStatement stmt = conn.prepareStatement(SQL_SELECT_SAMPLES_STATUS)) {
                ...
                try (ResultSet rs = stmt.executeQuery()) {
                    while (rs.next()) {
                        ...
                        kitStatuses.add(new KitStatus(rs.getString(DBConstants.KIT_TYPE_NAME), trackingId,
                                trackingId != null ? rs.getString(DBConstants.DSM_CARRIER_TO) : null,
                                sent != null ? (Long) sent / 1000 : null, delivered != null ? (Long) sent / 1000 : null,
            ...
            try (PreparedStatement stmt = conn.prepareStatement(SQL_SELECT_BATCH_SAMPLES_STATUS)) {
                ...
                try (ResultSet rs = stmt.executeQuery()) {
                    while (rs.next()) {
                        ...
                        kitStatuses.add(
                                new KitStatus(rs.getString("ddp_kit_request_id"), rs.getString(DBConstants.KIT_TYPE_NAME), trackingId,
                                        trackingId != null ? rs.getString(DBConstants.DSM_CARRIER_TO) : null,
                                        sent != null ? (Long) sent / 1000 : null, delivered != null ? (Long) sent / 1000 : null,
                                        received != null ? (Long) received / 1000 : null));
