...
public interface JdbiUserRole extends SqlObject {
    ...
    @SqlQuery(
            "SELECT u.guid, u.user_id, up.first_name, up.last_name, concat(up.first_name, \" \", up.last_name) as name, up.email, "
                    + "up.phone as phoneNumber,"
                    + " u.auth0_user_id, u.hruid as shortId, u.is_active, u.dsm_legacy_id, r.name as roleName, r.role_id, r.description, "
                    + "r.umbrella_id "
                    + "FROM user u "
