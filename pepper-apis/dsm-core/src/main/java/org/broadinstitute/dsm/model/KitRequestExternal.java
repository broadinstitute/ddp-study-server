...
public class KitRequestExternal extends KitRequest {
        ...
        try (PreparedStatement stmt = conn.prepareStatement(SQL_UPDATE_KIT_REQUEST_EXTERNAL_SHIPPER_STATUS)) {
            ...
            if (result > 1) {
                throw new RuntimeException(
                        "Error updating kit request w/ dsm_kit_request_id " + dsmKitRequestId + " it was updating " + result + " rows");
            }
        } catch (Exception e) {
    ...
    public static void updateKitRequestResponse(@NonNull Connection conn, String trackingIdTo, String trackingIdReturn, String kitLabel,
        ...
        try (PreparedStatement stmt = conn.prepareStatement(SQL_UPDATE_KIT_EXTERNAL_SHIPPER)) {
            ...
            if (result != 1) {
                throw new RuntimeException(
                        "Error updating kit w/ dsm_kit_request_id " + dsmKitRequestId + " it was updating " + result + " rows");
            }
        } catch (Exception e) {
        ...
        try {
            ...
            logger.error(
                    String.format("Error updating kit request shipping with dsm kit request id: %s in ElasticSearch", dsmKitRequestId));
            e.printStackTrace();
        }
