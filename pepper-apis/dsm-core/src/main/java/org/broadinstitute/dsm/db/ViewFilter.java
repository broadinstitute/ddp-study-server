...
public class ViewFilter {
    ...
    public static final String SQL_INSERT_VIEW =
            "INSERT INTO view_filters (view_columns, display_name, created_by, shared, query_items, parent,  quick_filter_name, ddp_group_id, changed_by, last_changed, deleted) " +
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    public static final String DESTRUCTION_QUEUE_QUERY =
            " AND oD.request <> 'received' AND oD.request <> 'no' AND oD.request <> 'hold' AND oD.request <> 'returned' AND oD.date_px IS NOT NULL AND oD.destruction_policy IS NOT NULL " +
                    ...
                    switch (state) {
                        ...
                        case 1:
                            if (word.equals(Filter.EQUALS_TRIMMED)) { // exact match selected in the frontend
                                ...
                                state = 3;
                                break;
                            } else if (word.equals(Filter.SMALLER_EQUALS_TRIMMED) ||
                                    word.equals(Filter.LARGER_EQUALS_TRIMMED)) { // range selected in the frontend
                                exact = false;
                                range = true;
                                state = 4;
                        ...
                        case 8:// query contained word "LIKE", exact match is false then
                            exact = false;
                            if (word.equals("'1'") ||
                                    (word.equals("1") && (Filter.CHECKBOX.equals(type)))) {// check boxes are either 1 or 0
            ...
            if (filters.containsKey(columnName) && (type == null || !type.equals(Filter.ADDITIONAL_VALUES))) {
                ...
                if (Filter.ADDITIONAL_VALUES.equals(filter.type)) {
                    ...
                    if (Filter.NUMBER.equals(filter.type)
                            && condition.contains(Filter.SMALLER_EQUALS_TRIMMED) &&
                            !arrayContains(conditions, Filter.LARGER_EQUALS_TRIMMED)) {
                        filter.setFilter2(new NameValue(columnName, value));
                    } else {
                        filter.setFilter1(new NameValue(columnName, value));
                    }
    ...
    public static String changeFieldsInQuery(String filterQuery, boolean savingFilter) {
        ...
        for (int i = 0; i < words.length; i++) {
            ...
            if (PatchUtil.getColumnNameMap().containsKey(word)) {
                words[i] = PatchUtil.getColumnNameMap().get(word).tableAlias + DBConstants.ALIAS_DELIMITER +
                        PatchUtil.getColumnNameMap().get(word).columnName;
