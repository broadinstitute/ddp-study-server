...
public class KitUtil {
    ...
    private static final String SQL_SELECT_KIT_REQUESTS_ALL_INFO =
            "SELECT *, cs_to.carrier as carrierTo, cs_to.easypost_carrier_id as carrierToId, cs_to.carrier_account_number as carrierToAccountNumber, cs_to.service as serviceTo, " +
    ...
    private static final String SQL_SELECT_COLLABORATOR_ID_KIT =
            "SELECT req.bsp_collaborator_participant_id FROM ddp_kit_request req WHERE req.ddp_participant_id = ? AND req.ddp_instance_id = ? LIMIT 1";
    ...
    private static final String SQL_SELECT_UNSENT_EXPRESS_KITS =
            "SELECT inst.ddp_instance_id, inst.instance_name, kType.kit_type_name, kType.required_role, (SELECT count(realm.instance_name) as kitRequestCount " +
        ...
        for (KitRequestCreateLabel kitLabelTriggered : kitsLabelTriggered) {
            ...
            try {
                ...
                throw new RuntimeException(
                        "Couldn't get address for participant " + kitLabelTriggered.getDdpParticipantId() + " in study " +
