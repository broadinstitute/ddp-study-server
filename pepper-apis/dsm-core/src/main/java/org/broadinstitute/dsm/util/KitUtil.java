...
public class KitUtil {
    ...
    public static final String SQL_UPDATE_KIT_RECEIVED = "UPDATE ddp_kit kit INNER JOIN( SELECT dsm_kit_request_id, MAX(dsm_kit_id) AS "
            + "kit_id " +
    ...
    private static final String SQL_SELECT_KIT_REQUESTS_ALL_INFO = "SELECT *, cs_to.carrier as carrierTo, cs_to.easypost_carrier_id as "
            + "carrierToId, cs_to.carrier_account_number as carrierToAccountNumber, cs_to.service as serviceTo, " +
            "cs_return.carrier as carrierReturn, cs_return.easypost_carrier_id as carrierReturnId, cs_return.carrier_account_number as "
            + "carrierReturnAccountNumber, " +
            "cs_return.service as serviceReturn FROM(SELECT kt.kit_type_name, ddp_site.instance_name, ddp_site.ddp_instance_id, ddp_site"
            + ".base_url, ddp_site.auth0_token, ddp_site.billing_reference, " +
            "ddp_site.collaborator_id_prefix, ddp_site.es_participant_index ,req.bsp_collaborator_participant_id, req"
            + ".bsp_collaborator_sample_id, req.ddp_participant_id, req.ddp_label, req.dsm_kit_request_id, " +
            "req.kit_type_id FROM kit_type kt, ddp_kit_request req, ddp_instance ddp_site where req.ddp_instance_id = ddp_site"
            + ".ddp_instance_id AND req.kit_type_id = kt.kit_type_id) " +
            "as request LEFT JOIN (SELECT * FROM (SELECT kit.dsm_kit_request_id, kit.dsm_kit_id, kit.kit_complete, kit.label_url_to, kit"
            + ".label_url_return, kit.tracking_to_id, kit.tracking_return_id, " +
            "kit.easypost_tracking_to_url, kit.easypost_tracking_return_url, kit.easypost_to_id, kit.scan_date, kit.label_date, kit"
            + ".error, kit.message, kit.receive_date, kit.deactivated_date, " +
            "kit.easypost_address_id_to, kit.deactivation_reason, tracking.tracking_id, kit.kit_label, kit.express FROM ddp_kit kit INNER"
            + " JOIN( SELECT dsm_kit_request_id, MAX(dsm_kit_id) AS kit_id " +
            "FROM ddp_kit GROUP BY dsm_kit_request_id) groupedKit ON kit.dsm_kit_request_id = groupedKit.dsm_kit_request_id AND kit"
            + ".dsm_kit_id = groupedKit.kit_id " +
            "LEFT JOIN ddp_kit_tracking tracking ON (kit.kit_label = tracking.kit_label))as wtf) as kit on kit.dsm_kit_request_id = "
    ...
    private static final String SQL_SELECT_COLLABORATOR_ID_KIT = "SELECT req.bsp_collaborator_participant_id FROM ddp_kit_request req "
            + "WHERE req.ddp_participant_id = ? AND req.ddp_instance_id = ? LIMIT 1";
    private static final String SQL_SELECT_COLLABORATOR_ID_TISSUE = "SELECT tis.collaborator_sample_id FROM ddp_medical_record med " +
            "LEFT JOIN ddp_institution inst on (med.institution_id = inst.institution_id) LEFT JOIN ddp_participant as part on (part"
            + ".participant_id = inst.participant_id) " +
            "LEFT JOIN ddp_onc_history_detail onc on (med.medical_record_id = onc.medical_record_id) LEFT JOIN ddp_tissue tis on (tis"
            + ".onc_history_detail_id = onc.onc_history_detail_id) " +
            "WHERE NOT med.deleted <=> 1 AND part.ddp_participant_id = ? AND part.ddp_instance_id = ? AND tis.collaborator_sample_id IS "
            + "NOT NULL LIMIT 1";
    private static final String SQL_UPDATE_COLLABORATOR_IDS = "UPDATE ddp_kit_request set bsp_collaborator_participant_id = ?, "
            + "bsp_collaborator_sample_id = ? WHERE dsm_kit_request_id = ?";
    private static final String SQL_SELECT_UNSENT_EXPRESS_KITS = "SELECT inst.ddp_instance_id, inst.instance_name, kType.kit_type_name, "
            + "kType.required_role, (SELECT count(realm.instance_name) as kitRequestCount " +
            "FROM ddp_kit_request request LEFT JOIN ddp_instance realm on request.ddp_instance_id = realm.ddp_instance_id " +
            "LEFT JOIN ddp_kit kit on request.dsm_kit_request_id = kit.dsm_kit_request_id LEFT JOIN kit_type kt on request.kit_type_id = "
            + "kt.kit_type_id " +
            "LEFT JOIN ddp_participant_exit ex on (request.ddp_participant_id = ex.ddp_participant_id AND request.ddp_instance_id = ex"
            + ".ddp_instance_id) " +
            "WHERE realm.instance_name = inst.instance_name AND request.kit_type_id = kType.kit_type_id AND ex.ddp_participant_exit_id IS"
            + " NULL " +
            "AND NOT (kit.kit_complete <=> 1) AND kit.express = 1 AND kit.deactivated_date is null) as kitRequestCount, (SELECT count"
            + "(role.name) " +
            "FROM ddp_instance realm, ddp_instance_role inRol, instance_role role WHERE realm.ddp_instance_id = inRol.ddp_instance_id " +
            "AND inRol.instance_role_id = role.instance_role_id AND role.name = ? AND realm.ddp_instance_id = inst.ddp_instance_id) as "
            + "'has_role' " +
    ...
    private static final String SQL_UPDATE_KIT = "UPDATE ddp_kit SET easypost_shipment_status = ?, easypost_shipment_date = ?, message = "
            + "? WHERE dsm_kit_id = ?";
        ...
        for (KitRequestCreateLabel kitLabelTriggered : kitsLabelTriggered) {
            ...
            try {
                if (StringUtils.isBlank(kitLabelTriggered.getAddressIdTo())) {
                    ...
                    DDPParticipant ddpParticipant = ElasticSearchUtil.getParticipantAsDDPParticipant(participantESData,
                    ...
                    if (ddpParticipant != null) {
                        ...
                        KitRequestShipping.deactivateKitRequest(kitLabelTriggered.getDsmKitRequestId(), "Participant not found", null,
                                "System");
                        ...
                        if (StringUtils.isNotBlank(kitLabelTriggered.getBaseURL())) {
                            ...
                            if (StringUtils.isNotBlank(kitLabelTriggered.getParticipantIndexES())) {
                                ...
                                ddpParticipant = DDPParticipant.getDDPParticipant(kitLabelTriggered.getBaseURL(),
                                        kitLabelTriggered.getInstanceName(),
    ...
    public static List<KitRequestCreateLabel> getListOfKitsLabelTriggered() {
        ...
        SimpleResult results = inTransaction((conn) -> {
            ...
            try (PreparedStatement bspStatement =
                         conn.prepareStatement(SQL_SELECT_KIT_REQUESTS_ALL_INFO.concat(QueryExtension.KIT_LABEL_TRIGGERED))) {
                    ...
                    while (rs.next()) {
                        kitsLabelTriggered.add(new KitRequestCreateLabel(rs.getString(DBConstants.DSM_KIT_ID),
                                ...
                                new KitRequestSettings(rs.getString(DBConstants.DSM_CARRIER_TO),
                                        ...
                                        rs.getString(DBConstants.DSM_CARRIER_RETURN),
        ...
        if (results.resultException != null) {
            throw new RuntimeException("Error getting list of kit requests which are triggered for label creation ",
    ...
    private static void buyShipmentForKit(@NonNull EasyPostUtil easyPostUtil, @NonNull String dsmKitId,
        ...
        try {
            ...
            participantShipment = KitRequestShipping.getShipment(easyPostUtil, billingReference, kitType, kitRequestSettings, false,
                    toAddress);
        } catch (Exception e) {
        ...
        try {
            ...
            returnShipment = KitRequestShipping.getShipment(easyPostUtil, billingReference, kitType, kitRequestSettings, true,
                    returnAddress);
        } catch (Exception e) {
    ...
    public static void updateCollaboratorIds(@NonNull KitRequestCreateLabel kitLabelTriggered, @NonNull String collaboratorParticipantId,
                                             @NonNull String bspCollaboratorSampleType) {
        SimpleResult results = inTransaction((conn) -> {
            String collaboratorSampleId = KitRequestShipping.generateBspSampleID(conn, collaboratorParticipantId,
                    bspCollaboratorSampleType, kitLabelTriggered.getKitTyp().getKitTypeId());
        ...
        if (results.resultException != null) {
            throw new RuntimeException("Error updating collaborator ids for kit request w/ id " + kitLabelTriggered.getDsmKitRequestId(),
                    results.resultException);
        }
    }
                                    ...
                                    if (StringUtils.isNotBlank(status)) {
                                        if (EASYPOST_DELIVERED_STATUS.equals(status) || EASYPOST_FAILURE_STATUS.equals(status) ||
                                            ...
                                            SimpleResult result = updateKit(conn, status, deliveredDate, kitMessage,
                                                    kitRequest.getDsmKitId());
                                            if (result.resultException != null) {
                                                logger.error("Error updating kit for kit request " + kitRequest.getDsmKitRequestId() + " "
                                                        + "of realm " + ddpInstance.getName(), result.resultException);
                                            }
    ...
    private static List<KitRequestShipping> getKitRequestsToCheckStatus(@NonNull String realm) {
        ...
        SimpleResult results = inTransaction((conn) -> {
            ...
            try (PreparedStatement stmt =
                         conn.prepareStatement(KitRequestShipping.SQL_SELECT_KIT_REQUEST + QueryExtension.BY_REALM + AND_EASYPOST_TO_ID)) {
        ...
        for (DDPInstance ddpInstance : ddpInstances) {
            ...
            if (ddpInstance.getInstanceSettings() != null && ddpInstance.getInstanceSettings().getKitBehaviorChange() != null) {
                ...
                if (ddpInstance.getInstanceSettings().getKitBehaviorChange() != null) {
                    ...
                    try {
                        uploaded =
                                kitBehavior.stream().filter(o -> o.getName().equals(InstanceSettings.INSTANCE_SETTING_UPLOADED)).findFirst().get();
                ...
                if (uploaded != null && StringUtils.isNotBlank(ddpInstance.getParticipantIndexES())) {
                    List<org.broadinstitute.dsm.db.KitType> kitTypes =
                            org.broadinstitute.dsm.db.KitType.getKitTypes(ddpInstance.getName(), null);
                    Map<String, Map<String, Object>> participants = ElasticSearchUtil.getDDPParticipantsFromES(ddpInstance.getName(),
                                    ...
                                    if (participant != null) {
                                        ...
                                        if (specialBehavior) {
                                            KitRequestShipping.deactivateKitRequest(kit.getDsmKitRequestId(),
                                                    SYSTEM_AUTOMATICALLY_DEACTIVATED + ": " + uploaded.getValue(),
                                                    DSMServer.getDDPEasypostApiKey(ddpInstance.getName()), "System");
                                            if (InstanceSettings.TYPE_NOTIFICATION.equals(uploaded.getType())) {
                                                String message = kitType.getName() + " kit for participant " + kit.getParticipantId() +
                                                        " (<b>" + kit.getCollaboratorParticipantId()
