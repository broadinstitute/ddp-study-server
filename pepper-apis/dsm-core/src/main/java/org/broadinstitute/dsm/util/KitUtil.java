...
public class KitUtil {
    ...
    public static final String SQL_UPDATE_KIT_RECEIVED =
            "UPDATE ddp_kit kit INNER JOIN( SELECT dsm_kit_request_id, MAX(dsm_kit_id) AS kit_id "
    ...
    private static final String SQL_SELECT_COLLABORATOR_ID_KIT =
            "SELECT req.bsp_collaborator_participant_id FROM ddp_kit_request req WHERE req.ddp_participant_id = ? "
                    + "AND req.ddp_instance_id = ? LIMIT 1";
    private static final String SQL_SELECT_COLLABORATOR_ID_TISSUE = "SELECT tis.collaborator_sample_id FROM ddp_medical_record med "
            ...
            + "WHERE NOT med.deleted <=> 1 AND part.ddp_participant_id = ? AND part.ddp_instance_id = ? "
            + "AND tis.collaborator_sample_id IS NOT NULL LIMIT 1";
    private static final String SQL_UPDATE_COLLABORATOR_IDS =
            "UPDATE ddp_kit_request set bsp_collaborator_participant_id = ?, bsp_collaborator_sample_id = ? WHERE dsm_kit_request_id = ?";
    private static final String SQL_SELECT_UNSENT_EXPRESS_KITS =
            "SELECT inst.ddp_instance_id, inst.instance_name, kType.kit_type_name, kType.required_role, "
                    ...
                    + "LEFT JOIN ddp_participant_exit ex on (request.ddp_participant_id = ex.ddp_participant_id "
                    + "AND request.ddp_instance_id = ex.ddp_instance_id) "
                    + "WHERE realm.instance_name = inst.instance_name AND request.kit_type_id = kType.kit_type_id "
                    + "AND ex.ddp_participant_exit_id IS NULL "
                    + "AND NOT (kit.kit_complete <=> 1) AND kit.express = 1 AND kit.deactivated_date is null) as kitRequestCount, "
                    ...
                    + "FROM ddp_instance inst, ddp_kit_request_settings kSetting, kit_type kType "
    ...
    private static final String SQL_UPDATE_KIT =
            "UPDATE ddp_kit SET easypost_shipment_status = ?, easypost_shipment_date = ?, message = ? WHERE dsm_kit_id = ?";
    ...
    public static List<KitRequestCreateLabel> getListOfKitsLabelTriggered() {
        ...
        if (results.resultException != null) {
            throw new DsmInternalError("Error getting list of kit requests which are triggered for label creation ",
    ...
    public static Map<String, KitRequestCreateLabel> getListOfSubKitsThatNeedLabels() {
        ...
        if (results.resultException != null) {
            throw new DsmInternalError("Error getting list of sub kit requests which are triggered for label creation ",
                    results.resultException);
        }
        logger.info("Found {} kit requests which should get a label", subKitsLabelTriggered.size());
        return subKitsLabelTriggered;
    }
            ...
            try (PreparedStatement stmt = conn.prepareStatement(SQL_UPDATE_COLLABORATOR_IDS)) {
                ...
                if (result != 1) {
                    throw new RuntimeException(
                            "Error updating collaborator ids for kit request w/ id " + kitLabelTriggered.getDsmKitRequestId()
                                ...
                                if (tracker != null) {
                                    ...
                                    if (StringUtils.isNotBlank(status)) {
                                        if (EASYPOST_DELIVERED_STATUS.equals(status) || EASYPOST_FAILURE_STATUS.equals(status)
                                            ...
                                            SimpleResult result =
                                                    updateKit(conn, status, deliveredDate, kitMessage, kitRequest.getDsmKitId(),
                                                            ddpInstanceDto);
    ...
    private static SimpleResult updateKit(@NonNull Connection conn, String status, Long date, String message, long dsmKitId,
        ...
        for (DDPInstance ddpInstance : ddpInstances) {
            ...
            if (ddpInstance.getInstanceSettings() != null && ddpInstance.getInstanceSettings().getKitBehaviorChange() != null) {
                ...
                if (ddpInstance.getInstanceSettings().getKitBehaviorChange() != null) {
                    ...
                    try {
                        uploaded =
                                kitBehavior.stream().filter(o -> o.getName().equals(InstanceSettings.INSTANCE_SETTING_UPLOADED)).findFirst()
                                        .get();
    ...
    public static String messageBasedOnLocationAndInstance(String errorMessage, DDPInstanceDto ddpInstanceDto, Address toAddress) {
        //overwriting the error message if study is PE-CGS and Participant is CA or NY otherwise keep the old error message
