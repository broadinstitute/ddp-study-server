...
public class MedicalRecordMigrationTool {
    ...
    public static final String SQL_SELECT_MEDICAL_RECORD_INFORMATION = "SELECT inst.institution_id, inst.ddp_institution_id, inst.type, "
            + "inst.participant_id, " +
            "part.ddp_participant_id, part.ddp_instance_id, ddp.instance_name, ddp.base_url, ddp.billing_reference, ddp"
            + ".es_participant_index, ddp.mr_attention_flag_d, ddp.tissue_attention_flag_d, ddp.auth0_token, " +
            "ddp.notification_recipients, ddp.migrated_ddp, m.medical_record_id, m.name, m.contact, m.phone, m.fax, m.fax_sent, m"
            + ".fax_sent_by, " +
            "m.fax_confirmed, m.fax_sent_2, m.fax_sent_2_by, m.fax_confirmed_2, m.fax_sent_3, m.fax_sent_3_by, m.fax_confirmed_3, m"
            + ".mr_received, m.follow_ups,  " +
            "m.mr_document, m.mr_document_file_names, m.mr_problem, m.mr_problem_text, m.unable_obtain, m.duplicate, m.followup_required,"
            + " m.followup_required_text, m.international, m.cr_required, m.pathology_present, " +
            "m.notes, (SELECT sum(log.comments is null and log.type = \"DATA_REVIEW\") as reviewMedicalRecord FROM ddp_medical_record "
            + "rec2 " +
            ...
            "FROM ddp_institution inst LEFT JOIN ddp_participant as part on (part.participant_id = inst.participant_id) LEFT JOIN "
            + "ddp_instance as ddp on (ddp.ddp_instance_id = part.ddp_instance_id) " +
    ...
    private static final String SQL_UPDATE_MEDICAL_RECORD = "UPDATE ddp_medical_record SET fax_sent = ?, fax_sent_by = ?, fax_confirmed ="
            + " ?, mr_received = ?, " +
...
//                        if (MedicalRecordUtil.isInstitutionInDB(conn, ddpParticipantId, ddpInstitutionId, instanceId, MBCInstitution
//                        .PHYSICIAN) == null) {
...
//                        }
                        ...
                        try (PreparedStatement stmt = conn.prepareStatement(SQL_SELECT_MEDICAL_RECORD_INFORMATION + " and inst"
                                + ".ddp_institution_id = \"" + ddpInstitutionId + "\"")) {
                        ...
                        if (StringUtils.isNotBlank(medicalRecordId) && StringUtils.isNotBlank(line.get(SENT_MR))) {
                            try (PreparedStatement stmt = conn.prepareStatement(SQL_UPDATE_MEDICAL_RECORD)) {
                                ...
                                stmt.setString(3, DBUtil.changeDateFormat(line.get(SENT_MR))); // medicalRecord.getFaxConfirmed()); at
                                // the moment, fax_confirmed = mr_received
