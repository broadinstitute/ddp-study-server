...
public class ElasticSearchAndDBComparison extends TestHelper {
    ...
    public void compareESAndDBMedicalRecords() throws Exception {
        ...
        Date date = new Date();
        File outputFile = new File(WORK_SPACE_PATH+"/ES_DB_COMPARE_"+STUDY_NAME+"_"+ENV+"_"+dateFormat.format(date)+".txt");
        ...
        try (RestHighLevelClient client = ElasticSearchUtil.getClientForElasticsearchCloud(cfg.getString("elasticSearch.url"),
            ...
            for (String esDdpParticipantId: esData.keySet()){
                ...
                for (HashMap esMedicalRecord: medicalRecords){
                    MedicalRecord dbMedicalRecord = getMedicalRecord(esMedicalRecord.get("medicalRecordId")+"");
    ...
    public static MedicalRecord getMedicalRecord(@NonNull String medicalRecordId) {
        SimpleResult results = inTransaction((conn) -> {
            SimpleResult dbVals = new SimpleResult();
            try (PreparedStatement stmt = conn.prepareStatement(
                    MedicalRecord.SQL_SELECT_MEDICAL_RECORD_SIMPLE )) {
