...
public class KitRequestMigrationTool {
        ...
        try {
            ...
            if (!carrierServiceTypes.isEmpty()) {
                ...
                if (kitRequestSettings != null) {
                    ...
                    migrateData(file, realmId, typeId, bspCollaboratorSampleType, collaboratorIdPrefix, fileFormat,
                            bspCollaboratorParticipantLength, realm);
                } else {
    ...
    private static void migrateData(@NonNull String jsonFile, @NonNull String realmId, @NonNull String typeId,
        ...
        if ("json".equals(fileFormat)) {
            ...
            for (JsonElement scan : scans) {
                ...
                insertIntoDB(cfg.getString(ApplicationConfigConstants.INSERT_KIT_REQUEST),
                        cfg.getString(ApplicationConfigConstants.INSERT_KIT),
            ...
            for (Map<String, String> line : content) {
                if (RGP.equals(realmName)) {
                    insertIntoDB(cfg.getString(ApplicationConfigConstants.INSERT_KIT_REQUEST),
                            ...
                            line.get(BSP_PARTICIPANT_ID), line.get(BSP_PARTICIPANT_ID), DBUtil.getLong(line.get(SHIPPED)),
                            DBUtil.getLong(line.get(DBUtil.RECEIVED)),
                    ...
                    insertIntoDB(cfg.getString(ApplicationConfigConstants.INSERT_KIT_REQUEST),
                            ...
                            bspCollaboratorSampleType, collaboratorIdPrefix, DBUtil.getLong(line.get(SHIPPED)),
                            DBUtil.getLong(line.get(DBUtil.RECEIVED)),
                            line.get(TRACKING_TO), line.get(TRACKING_RETURN));
                }
            }
        ...
        inTransaction((conn) -> {
            try {
                String collaboratorParticipantId =
                        KitRequestShipping.generateBspParticipantID(collaboratorIdPrefix, bspCollaboratorParticipantLength, shortId);
                insertIntoDB(insertKitRequestQuery, insertKitQuery, ddpParticipantId, shortId, realm, typeId, kitLabel,
                        ...
                        KitRequestShipping.generateBspSampleID(conn, collaboratorParticipantId, bspCollaboratorSampleType,
    ...
    private static void insertIntoDB(String insertKitRequestQuery, String insertKitQuery, String ddpParticipantId, String shortId,
                                     String realm, String typeId, String kitLabel, String bspCollaboratorParticipantId,
    ...
    private static void setToScanned(Connection conn, String updateQuery, String ddpLabel, String kitLabel, Long sendDate)
            throws Exception {
