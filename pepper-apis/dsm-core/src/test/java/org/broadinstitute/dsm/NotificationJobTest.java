...
public class NotificationJobTest extends TestHelper {
    ...
    private static String QUERY_UNSENT_REQUESTS =
            "select inst.ddp_instance_id, inst.instance_name, kType.kit_type_name, (select count(realm.instance_name) as kitRequestCount "
                    ...
                    + "and ex.ddp_participant_exit_id is null and not (kit.kit_complete <=> 1) and not (kit.error <=> 1) "
                    + "and not (kit.express <=> 1) and kit.deactivated_date is null and kit.label_url_to is not null) as kitRequestCount, "
                    ...
                    + "and role.name = \"kit_request_activated\" and realm.ddp_instance_id = inst.ddp_instance_id) as 'has_role' "
                    + "from ddp_instance inst, ddp_kit_request_settings kSetting, kit_type kType "
                    + "where inst.ddp_instance_id = kSetting.ddp_instance_id and kType.kit_type_id = kSetting.kit_type_id "
                    + "and inst.instance_name = ? and inst.is_active = 1 and kType.kit_type_name = ?;";
    private static String QUERY_NOTIFICATION_REQUESTS =
            "select * from EMAIL_QUEUE where EMAIL_RECORD_ID = ? and EMAIL_DATE_PROCESSED IS NULL";
    private static String DELETE_NOTIFICATION_REQUESTS =
    ...
    public void scheduleGPNotification() throws Exception {
        ...
        if (job != null) {
            ...
            SimpleTrigger trigger = TriggerBuilder.newTrigger().withIdentity(triggerKey).withSchedule(simpleSchedule()).build();
    ...
    public void getUnsentKits() {
        ...
        for (KitDDPSummary unsent : unsentKits) {
            ...
            Assert.assertEquals(DBTestUtil.getStringFromQuery(QUERY_UNSENT_REQUESTS, strings, "kitRequestCount"), unsent.getKitsQueue());
        }
    }
