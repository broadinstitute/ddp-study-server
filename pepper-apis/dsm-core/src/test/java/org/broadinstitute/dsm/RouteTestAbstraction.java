...
public class RouteTestAbstraction extends TestHelper {
    ...
    private static void cleanDB() {
        ...
        DBTestUtil.executeQuery("DELETE FROM medical_record_abstraction_field WHERE ddp_instance_id = (SELECT ddp_instance_id from "
                + "ddp_instance where instance_name = \"" + TEST_DDP + "\") AND display_name = 'New Field'");
        DBTestUtil.executeQuery("DELETE FROM medical_record_abstraction_field WHERE ddp_instance_id = (SELECT ddp_instance_id from "
                + "ddp_instance where instance_name = \"" + TEST_DDP + "\") AND display_name = 'New Field in Group'");
        DBTestUtil.executeQuery("DELETE FROM medical_record_abstraction_group WHERE ddp_instance_id = (SELECT ddp_instance_id from "
                + "ddp_instance where instance_name = \"" + TEST_DDP + "\") AND display_name = 'Unit Test Group New'");
    }

    private static void setupAbstractionForm() {
        //only add abstraction fields if TEST_DDP doesn't have them yet
        if (!DBTestUtil.checkIfValueExists("SELECT * from medical_record_abstraction_group where ddp_instance_id = (SELECT "
                + "ddp_instance_id from ddp_instance where instance_name = ?)", TEST_DDP)) {
            DBTestUtil.executeQuery("INSERT INTO medical_record_abstraction_group set ddp_instance_id = (SELECT ddp_instance_id from "
                    + "ddp_instance where instance_name = \"" + TEST_DDP + "\"), display_name=\"Unit Test Group 1\", order_number = 1");
            DBTestUtil.executeQuery("INSERT INTO medical_record_abstraction_field (display_name, type, "
                    + "medical_record_abstraction_group_id, ddp_instance_id, order_number, help_text) values " +
                    "('DOB', 'date', (SELECT medical_record_abstraction_group_id from medical_record_abstraction_group where "
                    + "display_name='Unit Test Group 1'), " +
            ...
            DBTestUtil.executeQuery("INSERT INTO medical_record_abstraction_field (display_name, type, possible_values, "
                    ...
                    "(SELECT medical_record_abstraction_group_id from medical_record_abstraction_group where display_name='Unit Test "
                    + "Group 1'), " +
                    "(SELECT ddp_instance_id from ddp_instance where instance_name = \"" + TEST_DDP + "\"), 2, 'Sex - Help text')");

            DBTestUtil.executeQuery("INSERT INTO medical_record_abstraction_group set ddp_instance_id = (SELECT ddp_instance_id from "
                    + "ddp_instance where instance_name = \"" + TEST_DDP + "\"), display_name=\"Unit Test Group 2\", order_number = 2 ");
            DBTestUtil.executeQuery("INSERT INTO medical_record_abstraction_field (display_name, type, possible_values, "
                    + "medical_record_abstraction_group_id, ddp_instance_id, order_number, help_text) values " +
                    "('Diagnostic Sample Type', 'select', '[{\"value\":\"valueI\"}, {\"value\": \"valueA\"}]', " +
                    "(SELECT medical_record_abstraction_group_id from medical_record_abstraction_group where display_name='Unit Test "
                    + "Group 2'), " +
                    "(SELECT ddp_instance_id from ddp_instance where instance_name =  \"" + TEST_DDP + "\"), 1, 'Diagnostic Sample Type -"
                    + " Help text')");
            DBTestUtil.executeQuery("INSERT INTO medical_record_abstraction_field (display_name, type, "
                    + "medical_record_abstraction_group_id, ddp_instance_id, order_number, help_text, possible_values) values " +
                    "('Medication', 'multi_type_array', (SELECT medical_record_abstraction_group_id from medical_record_abstraction_group"
                    ...
                    "'[{\"value\":\"Name of Drug\",\"type\":\"drugs\",\"type2\":\"select\"},{\"value\":\"Start\",\"type\":\"date\"},"
                    + "{\"value\":\"Stop\",\"type\":\"date\"},{\"value\":\"Trail\",\"type\":\"checkbox\"},{\"value\":\"Name\","
                    + "\"type\":\"text\"},{\"value\":\"Treatment Response\",\"type\":\"options\",\"values\":[{\"value\":\"stable\"},"
                    + "{\"value\":\"response\"},{\"value\":\"progression\"}]},{\"value\":\"DC Reason\",\"type\":\"options\","
            ...
            DBTestUtil.executeQuery("INSERT INTO medical_record_abstraction_field (display_name, type, "
                    ...
                    "(SELECT medical_record_abstraction_group_id from medical_record_abstraction_group where display_name='Unit Test "
                    + "Group 2'), " +
                    "(SELECT ddp_instance_id from ddp_instance where instance_name =  \"" + TEST_DDP + "\"), 4, 'Favorite number - Help "
                    + "text')");
            DBTestUtil.executeQuery("INSERT INTO medical_record_abstraction_field (display_name, type, "
                    + "medical_record_abstraction_group_id, ddp_instance_id, order_number, help_text, possible_values) values " +
                    ...
                    + " " +
                    "'[{\"value\":\"Location\",\"type\":\"options\",\"values\":[{\"value\":\"Brain\"},{\"value\":\"Pinkie\"},"
                    + "{\"value\":\"Toe\"}]},{\"value\":\"Date\",\"type\":\"date\"}]')");

            DBTestUtil.executeQuery("INSERT INTO medical_record_abstraction_group set ddp_instance_id = (SELECT ddp_instance_id from "
                    + "ddp_instance where instance_name = \"" + TEST_DDP + "\"), display_name=\"Unit Test Group 3\", order_number = 3 ");
            DBTestUtil.executeQuery("INSERT INTO medical_record_abstraction_field (display_name, type, "
                    + "medical_record_abstraction_group_id, ddp_instance_id, order_number, help_text) values " +
                    "('Text area', 'textarea', (SELECT medical_record_abstraction_group_id from medical_record_abstraction_group where "
                    + "display_name='Unit Test Group 3'), " +
    ...
    public void getFormControls() throws Exception {
        ...
        HttpResponse response = TestUtil.performGet(DSM_BASE_URL, "/ui/abstractionformcontrols?realm=" + TEST_DDP,
        ...
        String groups = DBTestUtil.getQueryDetail("select count(*) from medical_record_abstraction_group where ddp_instance_id = (SELECT "
                + "ddp_instance_id from ddp_instance where instance_name = ?)", TEST_DDP, "count(*)");
        ...
        String fields = DBTestUtil.getStringFromQuery("select count(*) from medical_record_abstraction_field where ddp_instance_id = "
                + "(SELECT ddp_instance_id from ddp_instance where instance_name = ?) AND medical_record_abstraction_group_id = (select "
                + "medical_record_abstraction_group_id from medical_record_abstraction_group where display_name = ?) ", strings, "count"
                + "(*)");
        ...
        fields = DBTestUtil.getStringFromQuery("select count(*) from medical_record_abstraction_field where ddp_instance_id = (SELECT "
                + "ddp_instance_id from ddp_instance where instance_name = ?) AND medical_record_abstraction_group_id = (select "
                + "medical_record_abstraction_group_id from medical_record_abstraction_group where display_name = ?) ", strings, "count"
                + "(*)");
    ...
    public void addGroup() throws Exception {
        ...
        List strings = new ArrayList<>();
        strings.add(TEST_DDP);
        String groups = DBTestUtil.getStringFromQuery("select count(*) from medical_record_abstraction_group where ddp_instance_id = "
                + "(SELECT ddp_instance_id from ddp_instance where instance_name = ?)", strings, "count(*)");
        ...
        response =
                TestUtil.performGet(DSM_BASE_URL, "/ui/abstractionformcontrols?realm=" + TEST_DDP, testUtil.buildAuthHeaders()).returnResponse();
        assertEquals(200, response.getStatusLine().getStatusCode());
        Gson gson = new GsonBuilder().create();
        AbstractionGroup[] abstractionGroups = gson.fromJson(DDPRequestUtil.getContentAsString(response), AbstractionGroup[].class);
        String newGroupsCount = DBTestUtil.getStringFromQuery("select count(*) from medical_record_abstraction_group where "
                + "ddp_instance_id = (SELECT ddp_instance_id from ddp_instance where instance_name = ?)", strings, "count(*)");
    ...
    private void addField(@NonNull String groupName, @NonNull String fieldName, @NonNull String fieldType, int orderNumber) throws Exception {
        ...
        String abstractionGroupId = DBTestUtil.getStringFromQuery("SELECT * from medical_record_abstraction_group where ddp_instance_id ="
                + " (SELECT ddp_instance_id from ddp_instance where instance_name = ?) and display_name = ?", strings,
        ...
        String json =
                "[{\"abstractionGroupId\":" + groupID + ", \"displayName\": \"" + groupName + "\", \"orderNumber\": " + orderNumber + ", "
                        + "\"changed\": true, \"fields\": [{\"newAdded\":true, \"displayName\":\"" + fieldName + "\", \"helpText\":\"" + fieldName + " - Help Text\", \"type\":\"" + fieldType + "\"}]}]";
        HttpResponse response = TestUtil.perform(Request.Patch(DSM_BASE_URL + "/ui/abstractionformcontrols?realm=" + TEST_DDP), json,
                testUtil.buildAuthHeaders()).returnResponse();
        Assert.assertEquals(200, response.getStatusLine().getStatusCode());
        ...
        boolean found = false;
        String newFieldsCount = DBTestUtil.getStringFromQuery("select count(*) from medical_record_abstraction_field where "
                + "ddp_instance_id = (SELECT ddp_instance_id from ddp_instance where instance_name = ?) AND "
                + "medical_record_abstraction_group_id = (select medical_record_abstraction_group_id from "
                + "medical_record_abstraction_group where display_name = ?) ", strings, "count(*)");
    ...
    public void abstraction() throws Exception {
        abstraction("abstraction", "{\\\"dateString\\\":\\\"1966-03-25\\\",\\\"est\\\":true}", "{\"dateString\":\"1966-03-25\","
                + "\"est\":true}");
        abstraction("review", "{\\\"dateString\\\":\\\"1966-03-02\\\",\\\"est\\\":true}", "{\"dateString\":\"1966-03-02\",\"est\":true}");
        //TODO start qc now
    }

    public void abstraction(@NonNull String activityName, @NonNull String abstractionUniqueValue,
                            @NonNull String abstractionUniqueValue2) throws Exception {
        String ddpParticipantId = "ABSTRACTION_PARTICIPANT_ID";
        ParticipantWrapperDto[] participants =
                RouteTest.getParticipants("/ui/applyFilter?parent=participantList&userId=26&realm=" + TEST_DDP);
        ...
        List strings = new ArrayList<>();
        strings.add(TEST_DDP);
        String fieldCount = DBTestUtil.getStringFromQuery("SELECT count(*) FROM medical_record_abstraction_field WHERE ddp_instance_id = "
                + "(SELECT ddp_instance_id FROM ddp_instance WHERE instance_name = ?)", strings, "count(*)");
        ...
        strings.add("DOB");
        strings.add(TEST_DDP);
        String fieldId = DBTestUtil.getStringFromQuery("SELECT * FROM medical_record_abstraction_field WHERE display_name = ? AND "
                + "ddp_instance_id = (SELECT ddp_instance_id FROM ddp_instance WHERE instance_name = ?)", strings,
        ...
        String medicationValue = "[{\"Name of Drug\":null,\"Start \":\"{\\\"dateString\\\":\\\"2019-10-01\\\",\\\"est\\\":false}\","
                + "\"Stop\":null,\"Trail\":null,\"Name\":null,\"Treatment Response\":\"other\",\"DC Reason\":null,\"other\":[{\"Treatment"
                + " Response\":\"some little response\"}]},{\"Name of Drug\":null,\"Start \":\"{\\\"dateString\\\":\\\"2019-12-03\\\","
                + "\\\"est\\\":false}\",\"Stop\":null,\"Trail\":null,\"Name\":null,\"Treatment Response\":null,\"DC Reason\":null}]";
        strings = new ArrayList<>();
        strings.add("Medication");
        strings.add(TEST_DDP);
        String medicationFieldId = DBTestUtil.getStringFromQuery("SELECT * FROM medical_record_abstraction_field WHERE display_name = ? "
                + "AND ddp_instance_id = (SELECT ddp_instance_id FROM ddp_instance WHERE instance_name = ?)", strings,
                "medical_record_abstraction_field_id");
        ...
        for (AbstractionGroup group : abstractionGroups) {
            for (AbstractionField field : group.getFields()) {
                if (!field.getDisplayName().equals("DOB") && !field.getDisplayName().equals("Medication")) {//or do it with id
                    ...
                    strings.add(TEST_DDP);
                    fieldId = DBTestUtil.getStringFromQuery("SELECT * FROM medical_record_abstraction_field WHERE display_name = ? AND "
                            + "ddp_instance_id = (SELECT ddp_instance_id FROM ddp_instance WHERE instance_name = ?)", strings,
                            "medical_record_abstraction_field_id");
        ...
        String medicationValueAfterSubmit = DBTestUtil.getStringFromQuery("SELECT * FROM ddp_medical_record_" + activityName + " WHERE "
                + "participant_id = ? AND medical_record_abstraction_field_id = ?", strings, "value");
        Assert.assertNotEquals(medicationValue, medicationValueAfterSubmit);
    }

    private void checkAbstractionValue(@NonNull AbstractionWrapper abstractionWrapperChanged, @NonNull String groupName,
                                       @NonNull String fieldName, @NonNull String value, @NonNull String activityName) throws Exception {
        boolean found = false;
    ...
    private void changeAbstractionStatus(@NonNull String ddpParticipantId, @NonNull String realm, @NonNull String activity,
        ...
        String activityId = DBTestUtil.getStringFromQuery("SELECT * FROM ddp_medical_record_abstraction_activities WHERE participant_id ="
                + " ? AND activity = ?", strings, "medical_record_abstraction_activities_id");
        String json = "{\"ddpParticipantId\":\"" + ddpParticipantId + "\", \"realm\": \"" + realm + "\", \"status\": \"" + newStatus +
                "\", \"userId\": \"1\", \"abstraction\": {\"participantId\":\"" + participantId + "\",\"user\":\"Name of user\", "
                + "\"activity\":\"" + activity + "\", \"aStatus\":\"" + currentStatus + "\"}}";

        if (activityId != null) {
            json = "{\"ddpParticipantId\":\"" + ddpParticipantId + "\", \"realm\": \"" + realm + "\", \"status\": \"" + newStatus + "\", "
                    + "\"userId\": \"1\", \"abstraction\": {\"participantId\":\"" + participantId + "\", "
                    + "\"medicalRecordAbstractionActivityId\":\"" + activityId + "\",\"user\":\"Name of user\", \"activity\":\"" + activity + "\", \"aStatus\":\"" + currentStatus + "\"}}";

        }
        //change abstraction aStatus
        HttpResponse response =
                TestUtil.perform(Request.Post(DSM_BASE_URL + "/ui/abstraction"), json, testUtil.buildAuthHeaders()).returnResponse();
        assertEquals(200, response.getStatusLine().getStatusCode());
        if (submitFail) {
    ...
    private AbstractionWrapper getAbstractionFields(@NonNull String ddpParticipantId, @NonNull String realm) throws Exception {
        ...
        AbstractionWrapper abstractionWrapper = new GsonBuilder().create().fromJson(DDPRequestUtil.getContentAsString(response),
                AbstractionWrapper.class);
