...
public class TestHelper {
    ...
    public static final String SELECT_KITREQUEST_QUERY = "SELECT dsm_kit_request_id FROM ddp_kit_request WHERE ddp_kit_request_id = ? AND"
            + " ddp_participant_id = ?";
    ...
    public static final String SELECT_EMAILQUEUE_QUERY = "select * from EMAIL_QUEUE where EMAIL_RECORD_ID = ? and REMINDER_TYPE = ? and "
            + "EMAIL_DATE_PROCESSED is null";
    public static final String SELECT_USER_SETTING = "select * from user_settings settings, access_user user where user.user_id = "
            + "settings.user_id and user.is_active = 1 and user.name = ?";
    ...
    public static final String CUSTOMS_JSON = "{\"description\": \"T-shirt\", \"quantity\": 1, \"value\": 11, \"weight\": 6, "
            + "\"origin_country\": \"US\", \"hs_tariff_number\": \"610910\", \"customs_certify\": true,\"customs_signer\": \"Jarrett "
            + "Streebin\", \"contents_type\": \"gift\", \"contents_explanation\":\"\", \"eel_pfc\": \"NOEEI 30.37(a)\", "
...
//        DBTestUtil.executeQuery("UPDATE ddp_instance set is_active = 1 where instance_name = \"" + TEST_DDP_MIGRATED + "\"");
        ...
        if (!DBTestUtil.checkIfValueExists("SELECT * from ddp_instance where instance_name = ?", TEST_DDP_MIGRATED)) {
            ...
            DBTestUtil.executeQuery("INSERT INTO ddp_kit_request_settings set ddp_instance_id = \"" + tmp + "\", kit_type_id = 1,
            kit_return_id = 1, carrier_service_to_id = 1, kit_dimension_id = 1 ");
        }
        //add second reminder
        ...
        "BLOOD_SENT_2WK")) {
            ...
            event_description=\"Blood kit - reminder email - 2 WKS\", kit_type_id=\"2\", event_type=\"REMINDER\", hours=\"336\"");
        }
        //adding test group
        ...
        List<String> roles = UserUtil.getUserRolesPerRealm("SELECT role.name FROM  access_user_role_group roleGroup LEFT JOIN
        ddp_instance_group gr on (gr.ddp_group_id = roleGroup.group_id) " +
                "LEFT JOIN access_user user on (roleGroup.user_id = user.user_id) LEFT JOIN ddp_instance realm on (realm.ddp_instance_id
                = gr.ddp_instance_id) " +
                "LEFT JOIN access_role role on (role.role_id = roleGroup.role_id) WHERE user.name = ? and instance_name = ?", "THE UNIT
                TESTER 1", TEST_DDP);
        //setting roles for test user for test group
        List<String> roles2 = UserUtil.getUserRolesPerRealm("SELECT role.name FROM  access_user_role_group roleGroup LEFT JOIN
        ddp_instance_group gr on (gr.ddp_group_id = roleGroup.group_id) " +
                "LEFT JOIN access_user user on (roleGroup.user_id = user.user_id) LEFT JOIN ddp_instance realm on (realm.ddp_instance_id
                = gr.ddp_instance_id) " +
                "LEFT JOIN access_role role on (role.role_id = roleGroup.role_id) WHERE user.name = ? and instance_name = ?", "THE UNIT
        ...
        if (!DBTestUtil.checkIfValueExists("SELECT * from instance_settings where ddp_instance_id = (SELECT ddp_instance_id from
        ddp_instance where instance_name = ?) ", TEST_DDP)) {
            ...
            \\\"name\\\":\\\"MD to MD exchange\\\", \\\"type\\\":\\\"checkbox\\\"}]\", ddp_instance_id = (SELECT ddp_instance_id from
            ddp_instance where instance_name = \"" + TEST_DDP + "\") ");
        }
    ...
    private static void checkRole(String role, List<String> roles, String user, String group) {
        if (!roles.contains(role)) {
            DBTestUtil.executeQuery("INSERT INTO access_user_role_group set user_id = " + user + ", group_id = " + group + ", role_id = "
                    + "(SELECT role_id from access_role where name = \"" + role + "\") ");
        }
    }
...
//                cfg.getString("portal.dbSslTrustStorePwd"));
    ...
    public static void setupEsClient() {
        if (esClient == null) {
            ...
            try {
                esClient = ElasticSearchUtil.getClientForElasticsearchCloud(cfg.getString("elasticSearch.url"), cfg.getString(
                        "elasticSearch.username"), cfg.getString("elasticSearch.password"));
    ...
    public static String addTissue(String oncHistoryId) throws Exception {
        ...
        String json = "{\"id\":null,\"parentId\":\"" + oncHistoryId + "\",\"parent\":\"oncHistoryDetailId\","
                + "\"user\":\"simone+1@broadinstitute.org\",\"nameValue\":{\"name\":\"t.tNotes\",\"value\":\"" + note + "\"}}";
        ...
        Assert.assertEquals(note, DBTestUtil.getStringFromQuery("select * from ddp_tissue where tissue_id = ? and notes = ? order by "
                + "last_changed desc limit 1", strings, "notes"));
        return tissueId;
    }
    ...
    public static void changeTissueValue(String tissueId, String oncHistoryId, String field, String value, String columnName) throws Exception {
        ...
        String json = "{\"id\":\"" + tissueId + "\",\"parentId\":\"" + oncHistoryId + "\",\"parent\":\"oncHistoryDetailId\","
                + "\"user\":\"ptaheri+1@broadinstitute.org\",\"nameValue\":{\"name\":\"" + field + "\",\"value\":\"" + value + "\"}}";
        HttpResponse response =
                TestUtil.perform(Request.Patch(DSM_BASE_URL + "/ui/patch"), json, testUtil.buildAuthHeaders()).returnResponse();
        Assert.assertEquals(200, response.getStatusLine().getStatusCode());
    ...
    public String editMedicalRecord(@NonNull String instanceName, @NonNull String participantId, @NonNull String institutionId,
        ...
        String medicalRecordId =
                DBTestUtil.getQueryDetail(MedicalRecord.SQL_SELECT_MEDICAL_RECORD + " and inst.ddp_institution_id = \"" + institutionId + "\" and p.ddp_participant_id = \"" + participantId + "\"", instanceName, "medical_record_id");
    ...
    public String addOncHistoryDetails(String participantId) throws Exception {
        //add oncHistoryDetail
        String json = "{\"id\":null,\"parentId\":\"" + participantId + "\",\"parent\":\"participantId\","
                + "\"user\":\"simone+1@broadinstitute.org\",\"nameValue\":{\"name\":\"oD.datePX\",\"value\":\"2017-01-02\"}}";
        HttpResponse response =
                TestUtil.perform(Request.Patch(DSM_BASE_URL + "/ui/patch"), json, testUtil.buildAuthHeaders()).returnResponse();
        Assert.assertEquals(200, response.getStatusLine().getStatusCode());
        String message = DDPRequestUtil.getContentAsString(response);
        ...
        json = "{\"id\":\"" + oncHistoryId + "\",\"parentId\":\"" + participantId + "\",\"parent\":\"participantId\","
                + "\"user\":\"simone+1@broadinstitute.org\",\"nameValue\":{\"name\":\"oD.typePX\",\"value\":\"typeTesty\"}}";
        ...
        //get last changed on with that combination to get the one just added
        Assert.assertEquals("2017-01-02", DBTestUtil.getStringFromQuery("select * from ddp_onc_history_detail where onc_history_detail_id"
                + " = ? and type_px = ? order by last_changed desc limit 1", strings, "date_px"));
    ...
    public void changeValue(String valueId, String participantId, String field, Object value, String columnName, String parent,
                            String user, String pkName, String tableName, String fieldId) throws Exception {
        //change value
        String json =
        ...
        if (String.valueOf(value).startsWith("[")) {//for abstraction value change
            json = "{\"id\":null,\"parentId\":\"" + participantId + "\",\"parent\":\"" + parent + "\",\"user\":\"" + user + "\",\"fieldId"
                    + "\":\"" + fieldId + "\",\"nameValue\":{\"name\":\"" + field + "\",\"value\":" + value + "}}";
        }
        HttpResponse response =
                TestUtil.perform(Request.Patch(DSM_BASE_URL + "/ui/patch"), json, testUtil.buildAuthHeaders()).returnResponse();
        Assert.assertEquals(200, response.getStatusLine().getStatusCode());
        ...
        if (fieldId == null && value instanceof String) {
            ...
            if (StringUtils.isNotBlank((String) value)) {
                Assert.assertEquals(value, DBTestUtil.getStringFromQuery("select * from " + tableName + " where " + pkName + " = ?",
                        strings, columnName));
            } else {
                Assert.assertNull(DBTestUtil.getStringFromQuery("select * from " + tableName + " where " + pkName + " = ?", strings,
                        columnName));
            }
        }
    }
