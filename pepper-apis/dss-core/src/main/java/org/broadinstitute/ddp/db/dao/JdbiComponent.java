...
public interface JdbiComponent extends SqlObject {
    ...
    @SqlQuery("select ct.component_type_code as component_type,"
            ...
            + "  join block_component as bc on c.component_id = bc.component_id"
            + "  join revision as rev on rev.revision_id = c.revision_id"
            + " where bc.block_id = :blockId and rev.end_date is null")
    ...
    Optional<ComponentDto> findActiveComponentByBlock(@Bind("blockId") long blockId);
    ...
    Stream<ComponentDto> findComponentDtosByIds(
            @BindList(value = "ids", onEmpty = EmptyHandling.NULL) Iterable<Long> componentIds);
    ...
    @SqlUpdate("update component set revision_id = :revisionId where component_id = :id")
