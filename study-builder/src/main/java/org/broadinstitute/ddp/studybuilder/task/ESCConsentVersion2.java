...
public class ESCConsentVersion2 implements CustomTask {
    ...
    public void init(Path cfgPath, Config studyCfg, Config varsCfg) {
        ...
        if (!studyCfg.getString("study.guid").equals(ESC)) {
            throw new DDPException("This task is only for the " + ESC + " study!");
        }
    ...
    private void revisionContentBlock(Handle handle, RevisionMetadata meta, ActivityVersionDto versionDto,
        ...
        String oldName = String.format("esc_%s_%s", key, part);
        String newName = String.format("esc_%s_v2_%s", key, part);
